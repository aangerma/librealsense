...
private:
    ...
    translation calculate_translation_x_coeff(double3 v, double rc, double2 xy, const calib& yuy_intrin_extrinn);
        ...
        for (auto j = 0; j < width; j++)
        {
            ...
            if (z_edge >= z_edge_ninus[idx] && z_edge >= z_edge_plus[idx])
            {
                if (ir_data.ir_edges[idx] > _params.grad_ir_threshold && z_data.edges[idx] > _params.grad_z_threshold)
                {
                    ...
                    subpixels_x.push_back((double)j + fraqStep[idx] * (double)dir_map[dir].first);
                    subpixels_y.push_back((double)i + fraqStep[idx] * (double)dir_map[dir].second);
...
auto_cal_algo::z_frame_data auto_cal_algo::preproccess_z(const ir_frame_data& ir_data, rs2_intrinsics depth_intrinsics, float depth_units)
{
    ...
    f.open("subpixels_y");
    f.precision(15);
    for (auto i = 0; i < res.subpixels_y.size(); i++)
    {
        f << std::fixed << res.subpixels_y[i] << std::endl;
    ...
    //std::sort(vertices.begin(), vertices.end(), [](double3 f, double3 s) {return f.x < s.x; });
   ...
   {
       f2 << std::fixed<<vertices[i].x << " " << vertices[i].y << " " << vertices[i].z << std::endl;
   }*/
...
auto_cal_algo::yuy2_frame_data auto_cal_algo::preprocess_yuy2_data(const rs2_intrinsics& rgb_intrinsics)
{
    ...
    auto YUY2_IDTx = get_image<double>("C:/work/librealsense/build/wrappers/opencv/imshow/3 - Copy/binFiles/YUY2_IDTx_1080x1920_double_00.bin", 1920, 1080);

    std::ofstream f;
    f.open("YUY2_IDTx_matlab");
    f.precision(15);
    for (auto i = 0; i < YUY2_IDTx.size(); i++)
    {
        f << std::fixed << YUY2_IDTx[i] << std::endl;
    ...
    f1.open("YUY2_IDTx");
    f1.precision(15);
    for (auto i = 0; i < res.edges_IDTx.size(); i++)
    {
        f1 << std::fixed << res.edges_IDTx[i] << std::endl;
    ...
    auto_cal_algo::calib curr_calib)
{
    ...
    {
        f <<std::fixed<< points[i].x << " " << points[i].y << " " << points[i].z << std::endl;
    ...
    const calib& curr_calib)
{
    ...
    std::ofstream f1;
    f1.open("interp_IDT_y");
    f1.precision(15);
    for (auto i = 0; i < interp_IDT_y.size(); i++)
    {
        f1 << interp_IDT_y[i] << std::endl;
    }
    f1.close();*/
...
auto_cal_algo::k_matrix auto_cal_algo::calc_k_gradients(const z_frame_data & z_data, const yuy2_frame_data & yuy_data, std::vector<double> interp_IDT_x, std::vector<double> interp_IDT_y, const calib & yuy_intrin_extrin, const std::vector<double>& rc, const std::vector<double2>& xy)
{
    ...
    for (auto i = 0; i < coefs.x_coeffs.size(); i++)
    {
        f << coefs.x_coeffs[i].fy << std::endl;
    }
...
std::pair<auto_cal_algo::calib, double> auto_cal_algo::calc_cost_and_grad(const z_frame_data & z_data, const yuy2_frame_data & yuy_data, const calib& curr_calib)
{
    ...
    {
        f << std::fixed << uvmap[i].x << " " << uvmap[i].y << std::endl;
...
bool auto_cal_algo::optimaize()
{
    ...
    std::cout.precision(15);
    std::cout << std::fixed << "Original cost = " << cost.second << std::endl;
    ...
    for (auto i = 0; i < _params.max_optimization_iters; i++)
    {
        ...
        std::cout.precision(15);
        std::cout << std::fixed << "Current optimaized cost = " << params_curr.cost << std::endl;
        std::cout << std::fixed << "fx = " << res.first.k_mat.fx << std::endl
            << "fy = " << res.first.k_mat.fy << std::endl
            << "ppx = " << res.first.k_mat.ppx << std::endl
            << "ppy = " << res.first.k_mat.ppy << std::endl;
        ...
        /* std::cout.precision(15);
         std::cout << std::fixed << "Current optimaized cost = " << params_curr.cost << std::endl;
         std::cout << std::fixed << "fx = " << params_curr.curr_calib.k_mat.fx << std::endl
             <<"fy = " << params_curr.curr_calib.k_mat.fy << std::endl
            << "ppx = " << params_curr.curr_calib.k_mat.ppx << std::endl
            << "ppy = " << params_curr.curr_calib.k_mat.ppy << std::endl;*/

        optimized = true;
    }
    std::cout.precision(15);
    std::cout << std::fixed << "Optimaized cost = " << params_curr.cost << std::endl;
